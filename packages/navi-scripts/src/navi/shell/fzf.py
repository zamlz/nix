        ...
        with NamedTemporaryFile() as ifp:
            with NamedTemporaryFile() as ofp:
                with open(ifp.name, 'w') as f:
                    f.writelines([f"{opt}\n" for opt in options])
                # Using os.system() instead of subprocess.run() because fzf needs
                # direct TTY access for keyboard input. Shell redirection handles
                # input/output through files while preserving terminal interactivity.
